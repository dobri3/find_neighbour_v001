...
Future<void> _leaveGroup() async {
  ...
  try {
    await ApiService.matcherService.leaveGroup(_group.id);

    setState(() {
      _isJoinButtonLoading = false;
      _group.members.removeWhere((m) => m.userId == _session!.id);
    });
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Не удалось выйти из группы: $e'),
      ),
    );
  }
}
...
Future<void> _sendJoinRequest() async {
  ...
  Widget build(BuildContext context) {
    if (_session == null || _group.id.isEmpty) {
  ...
  final bool isOwner = _session!.id == _group.ownerId;
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Center(
          ...
          isMember: _group.members.any((m) => m.userId == _session!.id), onLeave: _leaveGroup ,
          isOwner: isOwner,
          ),
                      ],
                    )
                    ...
                    children: [
                      for (var member in _group.members)
                          Member(
                            ...
                            onRemove: 
                            // () async {
                            //   await ApiService.matcherService.kickMember(_group.fjghgc); 
                            // },
                                                      () async {
                            if (member.userId == _session!.id) return;

                            await ApiService.matcherService.kickMember(
                              _group.id,
                              member.userId,
                            );

                            setState(() {
                              _group.members.removeWhere((m) => m.userId == member.userId);
                            });
                          }
                          )
                    ],
...
class _ContactsCard extends StatelessWidget {
  ...
  const _ContactsCard({
    ...
    required this.onJoin, required this.isMember, required this.onLeave, required this.isOwner,
  });
        ...
        children: [
          ...
          Column(
            children: [
              ...
              //                 style: AppTextStyles.redSmall(context),
              //               ),
              //           ),
              //         ),
              //   ),
